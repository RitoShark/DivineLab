@echo off
echo Starting Quartz with integrated backend...

REM Start the existing backend server (includes WAD extraction)
echo Starting integrated backend server...
start "Quartz Backend" cmd /k "python bumpath_backend_standalone_final.py"
...
start "Quartz Frontend" cmd /k "npm start"
